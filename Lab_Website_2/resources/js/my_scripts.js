	...
	var cellToUpdate;
		...
		{//Outer for loop iterates over each row
			...
			// {
				...
				cellToUpdate = table.rows[row_counter].cells[4];
				if(parseInt(cell_value) > parseInt(cell_value2)){
					cellToUpdate.innerHTML = "Cu Boulder"
					wins = wins + 1;
				}
				else{
					cellToUpdate.innerHTML = table.rows[row_counter].cells[1].innerHTML;
					losses = losses + 1;
				}
...
function loadPlayersPage(){
	...
	var playerIndex = 0;
	for(playerIndex; playerIndex < players.length ; playerIndex++){
		// var x = document.createElement("A");
		// x.setAttribute("href", "#");
		// x.setAttribute("innerHTML", players[playerIndex]);
		menu.innerHTML += "<a class = 'dropdown-item' href = '#' onClick = 'switchPlayers(" + playerIndex.toString()+ ")'>" + players[playerIndex].name + "</a>"
